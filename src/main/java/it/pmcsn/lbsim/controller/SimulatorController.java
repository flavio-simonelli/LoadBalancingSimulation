...
public class SimulatorController {
    ...
    public void startSimulation(SimConfiguration config) {
        ...
        switch (config.getRunType()) {
            ...
            case REALSIMULATION -> realsimulation(config);
        }
    }

    public void realsimulation(SimConfiguration config) {
        // istanza random number generator e pianta i semi
        Rngs rngs = istanceRandomGenerator(config.getSeed());
        // crea workload con fasi
        List<Phase> phases = config.getRealSimulationPhases();
        // inizializza distribuzioni service e interarrival (dummy iniziale, verrà sostituito con la prima fase)
        HyperExponential interarrival = new HyperExponential(
                config.getInterarrivalCv(),
                phases.getFirst().getMeanInterarrival(),
                config.getInterarrivalStreamP(),
                config.getInterarrivalStreamHexp1(),
                config.getInterarrivalStreamHexp2()
        );
        HyperExponential service = new HyperExponential(
                config.getServiceCv(),
                config.getServiceMean(),
                config.getServiceStreamP(),
                config.getServiceStreamHexp1(),
                config.getServiceStreamHexp2()
        );
        // workload generator con fasi
        RealSimulationWorkloadGenerator wg = new RealSimulationWorkloadGenerator(rngs, interarrival, service, phases);
        ...
        for (int replica = 0; replica < config.getNumberOfReplicas(); replica++) {
            ...
            Simulator simulator = createNewSimulator(
                    ...
                    config.isSpikeEnabled(),
                    config.getSImax(),
                    config.isHorizontalEnabled(),
                    config.getSlidingWindowSize(),
                    config.getR0min(),
                    config.getR0max(),
                    config.getHorizontalCoolDown(),
                    runPolicy,
                    wg
            );

            double simDuration = phases.get(phases.size() - 1).getEndTime();
            if (Double.isInfinite(simDuration)) {
                simDuration = config.getDurationInSeconds().getSeconds();
            }

            simulator.run(simDuration);

            logger.log(Level.SEVERE, "Final seeds of Replica {0}: {1}\n",
                    new Object[]{replica, Arrays.toString(rngs.getSeedArray())});

            // reset generator per la prossima replica
            wg.reset();
        }
        runPolicy.closeCsvs();
    }
    ...
    public void autocorrelation(SimConfiguration config) {
        ...
        Simulator simulator = createNewSimulator(config.getInitialServerCount(), config.getSpikeCpuMultiplier(), config.getSpikeCpuPercentage(), config.getSchedulingType(), config.isSpikeEnabled(), config.getSImax(), config.isHorizontalEnabled(), config.getSlidingWindowSize(), config.getR0min(), config.getR0max(), config.getHorizontalCoolDown(), runPolicy, wg);
        // run simulation
        simulator.run(config.getMaxLag()*50);
