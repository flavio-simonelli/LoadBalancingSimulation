            ...
            for (int i=0; i< config.getReplications(); i++) {
                ...
                switch (config.getChooseWorkload()) {
                    ...
                    case WorkloadType.FULLEXP:
                        ...
                        logger.log(Level.INFO, "Full exponential workload with interarrival mean {0} and stream {1} and service mean {2} and stream {3}\n", new Object[]{config.getInterarrivalMean(), config.getInterarrivalStreamP(), config.getServiceMean(), 1});
                        break;
                ...
                rngs.selectStream(0);
                try {
                    welfordCsv = new CsvAppender(Path.of("output/csv/Welford" + rngs.getSeed() + "rep" + i + "j" + j + ".csv"), "Type", "N", "Mean", "StdDev", "Variance", "Semi Intervallo media");
                } catch (IOException e) {
                    throw new RuntimeException(e);
                }
                try {
                    jobStatsCsv = new CsvAppender(Path.of("output/csv/Jobstats" + rngs.getSeed() + "rep" + i + "j" + j + ".csv"), "JobID", "Arrival", "Departure", "ResponseTime", "OriginalSize", "AssignedServerID");
                } catch (IOException e) {
                    throw new RuntimeException(e);
                }
                try {
                    serverStatsCsv = new CsvAppender(Path.of("output/csv/Serverstats" + rngs.getSeed() + "rep" + i + "j" + j + ".csv"), "Time", "ServerID", "State", "ActualSI");
                } catch (IOException e) {
                    throw new RuntimeException(e);
                }
                try {
                    departureStatsCsv = new CsvAppender(Path.of("output/csv/Departures" + rngs.getSeed() + "rep" + i + "j" + j + ".csv"), "Time", "NumJobs", "ProxArrival", "ProxDepartureofAllJobs", "RemainingSizeOfAllJobs", "NumServers");
                } catch (IOException e) {
                    throw new RuntimeException(e);
                }
                ...
                double originalTime = config.getDurationSeconds().getSeconds();
                double updatedTime = originalTime * (Math.pow(2, j));
                simulator.run(updatedTime);
                // print final seed
