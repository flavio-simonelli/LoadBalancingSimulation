...
public class ConfigLoader {
    ...
    private static class ConfigAdapter implements SimConfiguration {
        ...
        public RunType getRunType() {
            return RunType.fromString(cfg.simulation.typesimulation);
        }
        ...
        @Override
        public boolean getIsTracedriven() {
            return cfg.workload.isTracedriven;
        }

        @Override
        public Duration getDurationInSeconds() {
            return cfg.finitesimulation.duration;
        }

        @Override
        public int getDurationInJobs() {
            return cfg.finitesimulation.numjobs;
        }

        @Override
        public int getNumberOfBatchs() {
            return cfg.infinitesimulation.k;
        }

        @Override
        public int getNumberOfReplicas() {
            return cfg.finitesimulation.replica;
        }

        @Override
        public int getBatchSize() {
            return cfg.infinitesimulation.b;
        }

        @Override
        public int getMaxLag() {
            return cfg.autocorrelation.maxlag;
        }

        @Override
        public double getInterarrivalMean() {
            return cfg.workload.interarrival.mean;
        }

        @Override
        public double getInterarrivalCv() {
            return cfg.workload.interarrival.cv;
        }

        @Override
        public int getInterarrivalStreamP() {
            return cfg.workload.interarrival.streamp;
        }

        @Override
        public int getInterarrivalStreamHexp1() {
            return cfg.workload.interarrival.streamhexp1;
        }

        @Override
        public int getInterarrivalStreamHexp2() {
            return cfg.workload.interarrival.streamhexp2;
        }

        @Override
        public double getServiceMean() {
            return cfg.workload.service.mean;
        }

        @Override
        public double getServiceCv() {
            return cfg.workload.service.cv;
        }

        @Override
        public int getServiceStreamP() {
            return cfg.workload.service.streamp;
        }

        @Override
        public int getServiceStreamHexp1() {
            return cfg.workload.service.streamhexp1;
        }

        @Override
        public int getServiceStreamHexp2() {
            return cfg.workload.service.streamhexp2;
        }
        ...
        @Override
        public boolean isSpikeEnabled() {
            return cfg.scaling.spikeServer.enabled;
        }

        @Override
        public int getSImax() {
            return cfg.scaling.spikeServer.SImax;
        }

        @Override
        public int getSpikeCpuMultiplier() {
            return cfg.scaling.spikeServer.cpuMultiplier;
        }

        @Override
        public double getSpikeCpuPercentage() {
            return cfg.scaling.spikeServer.cpuPercentage;
        }

        @Override
        public boolean isHorizontalEnabled() {
            return cfg.scaling.horizontal.enabled;
        }

        @Override
        public int getSlidingWindowSize() {
            return cfg.scaling.horizontal.slidingWindowSize;
        }

        @Override
        public Duration getR0max() {
            return cfg.scaling.horizontal.R0max;
        }

        @Override
        public Duration getR0min() {
            return cfg.scaling.horizontal.R0min;
        }

        @Override
        public Duration getHorizontalCoolDown() {
            return cfg.scaling.horizontal.coolDown;
        }

        @Override
        public int getInitialServerCount() {
            return cfg.scaling.horizontal.initialServerCount;
        }

        @Override
        public String getCsvOutputDir() {
            return cfg.output.csvDir;
        }

        @Override
        public String getPlotOutputDir() {
            return cfg.output.plotDir;
        }

        @Override
        public String getTraceArrivalsPath() {
            return cfg.path.traceArrivalsPath;
        }

        @Override
        public String getTraceSizePath() {
            return cfg.path.traceSizePath;
        }

        @Override
        public WorkloadType getChooseWorkload() {
            return WorkloadType.fromString(cfg.workload.chooseWorkload);
        }

        @Override
        public List<Phase> getRealSimulationPhases() {
            if (cfg.realsimulation == null || cfg.realsimulation.phases == null) {
                return java.util.Collections.emptyList();
            }
            List<Phase> result = new ArrayList<>();
            List<Config.RealSimulation.Phase> rawPhases = cfg.realsimulation.phases;
            for (int i = 0; i < rawPhases.size(); i++) {
                double start = rawPhases.get(i).time.toSeconds();
                double end = (i + 1 < rawPhases.size())
                        ? rawPhases.get(i + 1).time.toSeconds()
                        : Double.POSITIVE_INFINITY; // ultima fase senza fine
                double meanInterarrival = rawPhases.get(i).interarrivalMean;
                result.add(new it.pmcsn.lbsim.models.simulation.Phase(start, end, meanInterarrival));
            }
            return result;
        }
